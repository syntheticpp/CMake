...
::WriteObjectBuildStatement(cmSourceFile* source)
{
  ...
  if(const char* objectLocationStr = source->GetProperty("OBJECT_LOCATION")) {
    std::string objectLocation = this->GetLocalGenerator()->Convert(objectLocationStr, cmLocalGenerator::HOME_OUTPUT);
    if (std::find(outputs.begin(), outputs.end(), objectLocation) == outputs.end()) {
      std::vector<std::string> outputList;
